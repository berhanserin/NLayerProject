...
namespace NLayerProject.Web.ApiService
{
    public class CategoryApiService
    {
        ...
        public async Task<IEnumerable<CategoryDto>> GetAllAsyncTask()
        {
        ...
        public async Task<CategoryDto> AddAsync(CategoryDto categoryDto)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                categoryDto = JsonConvert.DeserializeObject<CategoryDto>(await response.Content.ReadAsStringAsync());//clasa dönüştürme
                return categoryDto;
            }
            else
            {
